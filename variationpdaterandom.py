...
data_file = (
    BASE_DIR
    / "data"
    / "stable-isotopes-no-outliers.csv"
)
environment_file = (
    BASE_DIR
    / "data"
    / "temperature-and-light.csv"
)
...
data_muscle = df.dropna(
    subset=["Tissue Type"]
).copy()

# Exclude tissue type G

data_muscle = data_muscle[
    data_muscle["Tissue Type"] != "G"
].copy()

# Rename gear

data_muscle = data_muscle.rename(
    columns={
        "Gear Type": "Gear"
    }
)
...
data_muscle = data_muscle[
    data_muscle["Gear"].isin(
        ["C", "N", "W"]
    )
].copy()
print("\nInitial gear counts:")
print(
    data_muscle["Gear"].value_counts()
)

# CREATE COLLECTION DATE FROM CODE
...
# Convert original column to numeric code

data_muscle["Collection_Date_Code"] = pd.to_numeric(
    data_muscle["Collection Date"],
    errors="coerce"
)

# Map code to actual date

data_muscle["Collection_Date_Group"] = (
    data_muscle["Collection_Date_Code"]
    .map(collection_date_map)
)
...
# Remove observations with invalid collection date

data_muscle = data_muscle.dropna(
    subset=["Collection_Date_Group"]
).copy()

# Create month variable directly from collection date
...
# Convert environmental timestamp
...
if env["Date-Time (EDT)"].isna().any():
    ...
    print(
        "\nWARNING: Some environmental timestamps "
        "could not be parsed."
    )
...
def assign_environment(row):
    if row["Gear"] == "C":
        ...
        return pd.Series({
            "Temperature":
                row[
                    "Net Bottom, Temperature (°F)"
                ],
            "Light":
                row[
                    "Net Bottom, Light (lum)"
                ]
...
analysis_data["Collection_Date_Group"] = (
    analysis_data[
        "Collection_Date_Group"
    ].astype("category")
)
    ...
    for optimizer in optimizers:
        ...
        try:
            ...
            if result.converged:
                ...
                print(
                    f"    Successful optimizer: "
                    f"{optimizer}"
                )
                return result
            last_error = (
                f"{optimizer} did not converge"
            )
...
def variation_partitioning_lmm(
    ...
    if include_light:
        print(
            ...
            f"{response} — TEMPERATURE + LIGHT"
        )
    else:
        print(
            f"VARIATION PARTITIONING: "
            f"{response} — TEMPERATURE ONLY"
        )
    print("=" * 70)
    print(
    ...
    print(
        "Fitting environment model..."
    )
    environment_model = fit_lmm(
        subset,
        environment_formula,
        response
    )
    ...
    environment_marginal, environment_conditional = (
        mixed_model_r2(
            environment_model
        )
    )
    gear_marginal, gear_conditional = (
        mixed_model_r2(
            gear_model
        )
    )
    full_marginal, full_conditional = (
        mixed_model_r2(
            full_model
        )
    )
    null_marginal, null_conditional = (
        mixed_model_r2(
            null_model
        )
    )
    ...
    print(
        f"Environment marginal R²: "
        f"{environment_marginal:.4f}"
    )
    print(
        f"Gear marginal R²:        "
        f"{gear_marginal:.4f}"
    ...
    print(
        f"Full conditional R²:     "
        f"{full_conditional:.4f}"
    )
    ...
    print(
        f"Unique gear: "
        f"{unique_gear:.4f} "
        f"({unique_gear * 100:.2f}%)"
    )
    print(
        f"Shared environment + gear: "
        f"{shared:.4f} "
        f"({shared * 100:.2f}%)"
    )
    print(
    ...
    print(
        f"Collection-date variance: "
        f"{random_variance:.6f}"
    )
    print(
        f"Residual variance: "
        f"{residual_variance:.6f}"
    )
...
light_data = analysis_data[
    analysis_data["Gear"].isin(
        ["C", "N"]
    )
].copy()
print("\n\n")
...
light_results_df.to_csv(
    figures
    / "variation_partitioning_temperature_light_random_date_summary.csv",
    index=False
)
light_plot_data = (
    light_results_df[
...
light_plot_data.to_csv(
    figures
    / "variation_partitioning_temperature_light_random_date.csv",
    index=False
)
...
make_partition_plot(
    light_results_df,
    figures
    / "variation_partitioning_temperature_light_random_date.png",
...
temperature_data = analysis_data[
    analysis_data["Gear"].isin(
        ["C", "N", "W"]
    )
].copy()
...
temperature_results_df.to_csv(
    figures
    / "variation_partitioning_temperature_random_date_summary.csv",
    index=False
)
...
temperature_plot_data.to_csv(
    figures
    / "variation_partitioning_temperature_random_date.csv",
    index=False
)
...
make_partition_plot(
    temperature_results_df,
    figures
    / "variation_partitioning_temperature_random_date.png",
...
print("=" * 70)
print(
    "VARIATION PARTITIONING COMPLETE"
)
print("=" * 70)
print("\n")
print(
...
collection_date_summary.to_csv(
    figures
    / "collection_date_mapping.csv",
    index=False
)
...
print("\n")
print(
    "Figures and result tables saved to:"
)
print(
    figures
)
