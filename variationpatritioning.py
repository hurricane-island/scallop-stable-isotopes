...
data_muscle = df.dropna( # Keep tissue observations
    subset=["Tissue Type"]
).copy()

data_muscle = data_muscle[ # Exclude tissue type G
...
env["Month"] = ( # Extract month
    env["Date-Time (EDT)"].dt.month
)
...
def assign_environment(row):
    ...
    if row["Gear"] == "C":
        ...
        return pd.Series({
            "Temperature":
                row["Net Bottom, Temperature (°F)"],

            "Light":
                row["Net Bottom, Light (lum)"]
        })
...
analysis_data["Gear"] = (
    analysis_data["Gear"].astype("category")
)
...
print(
    analysis_data["Gear"].value_counts()
)
...
# VARIATION PARTITIONING FUNCTION
...
def variation_partitioning(
    ...
    r2_environment, environment_model = (
        model_r2(
            subset,
            response,
            environment_formula
...
light_results_df.to_csv(
    figures /
    "variation_partitioning_temperature_light_summary.csv",
    index=False
)
...
light_plot_data["Percent"] = (
    light_plot_data["Proportion"] * 100
)


light_plot_data.to_csv(
    figures /
    "variation_partitioning_temperature_light.csv",
    index=False
)
...
for component in plot_order:
    ...
    for response in responses:
        ...
        value = light_results_df.loc[
            light_results_df["Response"] == response,
            component
        ].iloc[0]
...
plt.savefig(
    figures /
    "variation_partitioning_temperature_light.png",
    dpi=300,
    bbox_inches="tight"
)
...
print("\nGear counts:")
print(
    temperature_data["Gear"].value_counts()
)
...
temperature_results_df.to_csv(
    figures /
    "variation_partitioning_temperature_summary.csv",
    index=False
)

temperature_plot_data = ( # Long-format version
...
temperature_plot_data["Percent"] = (
    temperature_plot_data["Proportion"] * 100
)

temperature_plot_data.to_csv(
    figures /
    "variation_partitioning_temperature.csv",
    index=False
)
...
for component in plot_order:
    ...
    for response in responses:
        ...
        value = temperature_results_df.loc[
            temperature_results_df["Response"] == response,
            component
        ].iloc[0]
...
plt.savefig(
    figures /
    "variation_partitioning_temperature.png",
    dpi=300,
    bbox_inches="tight"
)
